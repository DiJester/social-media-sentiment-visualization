...
/*  11-Mar-20	Christopher G. Healey	On rate limit, some tweets are	    */
/*					still returned, so process those    */
/*					so rate limit doesn't return no	    */
/*					results				    */
/*  11-Mar-20	Christopher G. Healey	Break multiple emojis with no	    */
/*					separators between them into	    */
...
var  block_sz = 1;			// Recent tweet pages/block queried
var  max_id;				// Current maximum twitter ID
var  max_pg = block_sz * 6;		// Max queries, MUST BE block_sz units
...
//  split( /[@&\?!\.,\-\+\/\|~'`\u2018\u2019"\u201C\u201D:;^\$\*\(\)\s]+/ );
  ...
  i = 0;
  while( i < tok.length ) {		// For all tokens

    //  Skip blank tokens and stop words
    ...
    //  Search for emoticons (they start with 0ud83d), because people
    //  often embed them or put in strings of them without separators,
    //  we need to manually make them individual tokens

    if ( ( pos = tok[ i ].search( '\ud83d' ) ) != -1 && tok[ i ].length > 2 ) {
      len = emoji_len( tok[ i ], pos );
      emoji = tok[ i ].substr( pos, len );

      prev = tok[ i ].substring( 0, pos );
      post = tok[ i ].substring( pos + len );

      if ( prev.length == 0 ) {		// Nothing preceeding emoji?
        tok[ i ] = emoji
        tok.splice( i + 1, 0, post );
      } else {
        tok[ i ] = prev;
        tok.splice( i + 1, 0, emoji );
        tok.splice( i + 2, 0, post );
      }
    }
  ...
  //  std:  Array of standard deviations
{
  ...
  std_avg = ( std_sum == 0 ) ?
    0 : Math.sqrt( ( std_sum / n ) - Math.pow( m_avg, 2.0 ) );
  ...
  for( i = 0; i < n; i++ ) {		// Calculate initial probs
    ...
    p.push( std[ i ] == 0 ?
      0: 1.0 / Math.sqrt( 2.0 * Math.PI * Math.pow( std[ i ], 2.0 ) ) );
    p_sum += p[ i ];
  }
  ...
  //  neg_tok:  Negated tokens in raw tweet
{
  ...
  var  sep_len;				// Assumed length of cur term separator
  ...
  //  Skip cur=0, which means tw_strip starts with a separator
  ...
  //  neg_tok:  Negated tokens in raw tweet
{
  ...
  tok = tw_body.toLowerCase().split(
      /[ \.,!\+"$%&^\\()*-\/:;<>?\[\]^{\|}~`@#\\\n]+/ );

  //  For every token, need to check for runs of emojis and break them
  //  into individual emoji characters

  i = 0;
  while( i < tok.length ) {
    if( tok[ i ].length <= 1 || !is_emoji( tok[ i ] ) ) {
      i++;
      continue;
    }

    tok.splice( i, 0, tok[ i ].substr( 0, 2 ) );
    tok[ i + 1 ] = tok[ i + 1 ].substr( 2 );
  ...
  //  Retrieve a block of pages from Twitter's v1.1 search API
{
  ...
  url = PHP_URL() + "recent.old.php?callback=?";
  //url = "https://www.csc.ncsu.edu/faculty/healey/tweet_viz/dev/php";
  //url = url + "recent.php?callback=?";
  url = url + "&q=" + term;
  url = url + "&pg=" + block_sz;
...
*/
        ...
        if ( data.http_code == 429 ) {	// Rate limited?
          console.log( "getJSON returns rate limit reached" );
          console.log( data );

          err_msg = "Twitter rate limit";
          exp_msg = "Twitter has rate limited the application, so few or no ";
          exp_msg += "results were returned. Please try your query again ";
          exp_msg += "in a few minutes";
	...
	//  Ensure after exiting conditional pg will be set > pg_req,
	//  forcing an update_load() to process any data received
