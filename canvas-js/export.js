  ...
  //  bar, to allow both to run in parallel
{
  ...
  //  Add all terms that were scored, along with their valence and
  //  arousal

  for( j = 0; j < tw[ i ].anew.length; j++ ) {
    line = line + tw[ i ].anew[ j ].word + ","
    line = line + tw[ i ].anew[ j ].avg[ VAL ] + ","
    line = line + tw[ i ].anew[ j ].avg[ ARO ]

    //  No comman on final term,arousal,valence

    if ( j < tw[ i ].anew.length - 1 ) {
      line = line + ","
    }
  }					// End for all ANEW terms in tweet
  ...
  //    -  date,user,body,valence,arousal,longitude,latitude
{
  ...
  line = "Date,User,Body,Valence,Arousal,Longitude,Latitude,";
  line = line + "Term(s),Valence,Arousal";
  CSV_line.push( line );
